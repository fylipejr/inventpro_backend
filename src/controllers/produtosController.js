...
async function cadastrarProduto(req, res) {
  ...
  if (!nome || !codigo || quantidade === undefined || !preco) {
    return res.status(400).json({ mensagem: 'Campos obrigatórios: nome, codigo, quantidade, preco.' });
  }
  if (quantidade < 0) {
    return res.status(400).json({ mensagem: 'Quantidade não pode ser negativa.' });
  }

  try {
    // Código único — Regra de Negócio RF03
    const [existente] = await db.query(
      'SELECT id_produto FROM mercadorias WHERE codigo = ?', [codigo]
    );
    if (existente.length > 0) {
      return res.status(409).json({ mensagem: 'Código de produto já existe. Cada produto deve ter código único.' });
    }
...
async function buscarProduto(req, res) {
  ...
  try {
    const [rows] = await db.query(
      'SELECT * FROM mercadorias WHERE id_produto = ?', [id]
    );
    if (rows.length === 0) {
  ...
  try {
    ...
    if (codigo && codigo !== existente[0].codigo) {
      const [dup] = await db.query(
        'SELECT id_produto FROM mercadorias WHERE codigo = ? AND id_produto != ?', [codigo, id]
      );
      if (dup.length > 0) {
        return res.status(409).json({ mensagem: 'Código de produto já existe.' });
      }
    }
...
async function excluirProduto(req, res) {
  ...
  try {
    const [existente] = await db.query(
      'SELECT id_produto FROM mercadorias WHERE id_produto = ?', [id]
    );
    if (existente.length === 0) {
      return res.status(404).json({ mensagem: 'Produto não encontrado.' });
    }
