...
async function registrarMovimentacao(req, res) {
  ...
  if (!id_produto || !tipo || !quantidade) {
    return res.status(400).json({ mensagem: 'Campos obrigatórios: id_produto, tipo, quantidade.' });
  }
  if (!['entrada', 'saida'].includes(tipo)) {
    return res.status(400).json({ mensagem: 'Tipo inválido. Use: entrada | saida' });
  }
  if (quantidade <= 0) {
  ...
  try {
    ...
    // Lock no produto para evitar concorrência
    ...
    // RF05 — Regra: sem estoque negativo
    if (tipo === 'saida' && produto.quantidade < quantidade) {
      await conn.rollback();
      return res.status(400).json({
        mensagem: `Estoque insuficiente. Disponível: ${produto.quantidade} unidades.`,
      });
    }
...
async function listarMovimentacoes(req, res) {
  ...
  if (data_inicio)  { sql += ' AND DATE(r.data_registro) >= ?'; params.push(data_inicio); }
  if (data_fim)     { sql += ' AND DATE(r.data_registro) <= ?'; params.push(data_fim); }
  if (tipo)         { sql += ' AND r.tipo = ?';                 params.push(tipo); }
  if (id_produto)   { sql += ' AND r.id_produto = ?';           params.push(id_produto); }
